...
SNE maps neighborhood structures by converting high-dimensional Euclidean distances into conditional probabilities that reflect local similarities. It drops these datapoints into a random 2d space. Then a loss function is used (KL Divergence) in order to assess how much our guessed 2d space is similar to the original high dimensional space, in other words it cares about how much datapoints are close together. At the end to optimise the loss in order to decrease the error as much as possible ADAM (Adaptive Moment Estimation) is used.   
